...
Assert-FileExists -Path $FfmpegPath -Message "Missing ffmpeg at: $FfmpegPath"
...
$filter = "[1:a]volume=1.0,atrim=0:60,apad=pad_dur=60,atrim=0:60,asetpts=PTS-STARTPTS[voice];[2:a]volume=0.17,atrim=0:60,apad=pad_dur=60,atrim=0:60,asetpts=PTS-STARTPTS[bgm];[voice][bgm]amix=inputs=2:duration=longest:dropout_transition=0,atrim=0:60,alimiter=limit=0.95[a]"
...
if ($process.ExitCode -ne 0) {
 throw "ffmpeg merge failed with exit code: $($process.ExitCode)"
}
